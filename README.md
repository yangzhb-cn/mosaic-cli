...
data/sessions/<id>.json   # messages、conversation_id、audit_records、model、时间戳
```
...
`Mosaic.md` 每轮动态读取并注入 `<system-reminder>`，不会写进 session messages。`conversations/` 保存按日期追加的 Markdown 对话归档，只记录顶层用户和助手回复，不记录子 Agent 内部对话，便于后续搜索历史。`/memory update` 会调用一次 LLM，把当前 session 中长期有效的信息提炼成新的完整 `Mosaic.md`；结果为空或调用失败时不会覆盖原文件。
...
`/act` 按 DAG 依赖调度，默认并发 4，并在 CLI 中用 `🔧 SubAgent(id=..., type=..., task=...)` 展示每个子 Agent 调用，随后输出完成、失败和重试进度。最终回复会写入每个 task 的结果摘要或失败原因，因此 session 能恢复执行结论，但不保存完整子 Agent 内部消息。`FILE_WRITE` 和 `COMMAND` 类型任务串行执行。某个 task 连续 3 次失败后停止调度新任务，保留已完成和失败结果；执行结束后自动退出 plan 状态。
...
同一个 session 多次保存会追加多行；不同 session 使用不同 `conversation_id` 文件。当前内存统计会随 session 自动保存和恢复，`/audit save` 仍只负责追加快照文件。
